...
export default function Layout({ children, currentPageName }) {
  ...
  const suggestedNextByCurrent = useMemo(() => ({
    Home: "Route Plan",
    Clients: "Route Plan",
    RouteOptimizer: "Clients",
    WorkOrders: "Home",
    WeeklyReport: "Route Plan",
    NewServiceLog: "Route Plan",
    ChemicalUsage: "Route Plan",
    PoolSchool: "Clients",
    Settings: "Route Plan",
    Notes: "Route Plan",
  }), []);

  const suggestedPage = suggestedNextByCurrent[currentPageName] || "Route Plan";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-100/50 font-sans selection:bg-cyan-100">
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 h-full w-64 bg-white/80 backdrop-blur-xl border-r border-slate-200/60 shadow-2xl flex-col z-40">
        ...
        <nav className="flex-1 p-4 space-y-1">
            ...
            const isSuggested = !active && item.name === suggestedPage;
            return (
              <Link
                key={item.name}
                to={item.path}
                aria-label={`${item.name}${active ? " (Current page)" : isSuggested ? " (Suggested next step)" : ""}`}
                title={active ? `${item.name} (current)` : isSuggested ? `Suggested next step: ${item.name}` : item.name}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${active
                  ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg border border-transparent"
                  : isSuggested
                    ? "text-slate-900 bg-cyan-50 border border-cyan-200 hover:bg-cyan-100"
              ...
              >
                ...
                {!active && isSuggested && (
                  <span className="ml-auto text-[10px] font-semibold text-cyan-700 bg-cyan-100 px-2 py-0.5 rounded-full">
                    Next
                  </span>
                )}
              </Link>
