...
const syncStateArb = fc.record({
  ...
  syncNow: fc.constant(vi.fn()),
  isRecordSynced: fc.constant(vi.fn()),
  getRecordSyncStatus: fc.constant(vi.fn()),
  refreshPendingCount: fc.constant(vi.fn()),
});
...
describe('SyncStatusBadge', () => {
  ...
  describe('Property 5: Record Sync Status Displayed Correctly', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (status) => {
            ...
            const badge = screen.getByText(status === 'synced' ? 'Synced' : 
    ...
    it('displays appropriate tooltip content for each status', () => {
      fc.assert(
        fc.property(
          recordSyncStatusArb,
          (status) => {
            ...
            // We can verify the component renders without errors and contains expected elements
            const badge = screen.getByText(status === 'synced' ? 'Synced' : 
                                         status === 'pending' ? 'Pending' : 'Error');
            expect(badge).toBeInTheDocument();

            // Verify the badge has the correct structure
