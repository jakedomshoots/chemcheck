...
export function ServicePhotoGallery({
  ...
  if (photos.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center text-ink-muted py-6">
        ...
        >
          <div className="relative max-w-4xl max-h-full">
            ...
            <button
              type="button"
              onClick={closeLightbox}
              className="absolute -top-12 right-0 p-2 text-white hover:text-ink-muted transition-colors focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-black rounded"
              ...
              <span
                className={`px-3 py-1 text-sm font-medium rounded-full capitalize text-white ${
                  lightboxPhoto.category === 'before'
                    ? 'bg-[var(--status-watch-soft)]0'
                    : 'bg-[var(--status-ok-soft)]0'
                }`}
              >
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
        {photos.map((photo) => (
          <button
            ...
            className="relative aspect-square rounded-lg overflow-hidden bg-surface-2 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 hover:opacity-90 transition-opacity"
          >
            <img
              src={photo.url}
